...
abstract class AsyncCommand<T>(var command: String, private var callback: AsyncCallback?): Command<Unit> {
    ...
    abstract fun parse(result: String): T
    ...
    override fun exec() {
        ...
        if(mProcess != null) {
            Thread {processStream(mProcess!!.inputStream)}.start()
            Thread {processStream(mProcess!!.errorStream)}.start()
    ...
    private fun processStream(stream: InputStream) {
